...
internal partial class ChatSession
{
    ...
    public static async Task RunMessageListRemoveTask()
    {
        while (true)
        {
            // System.Diagnostics.Trace.WriteLine("RunMessageListRemoveTask:個数:" + _contents.Count);
            if (autoRemoverCancelTokenSource != null && autoRemoverCancelTokenSource.IsCancellationRequested)
            {
                return;
            }

            // 質問追加⇒lastAnswerTime更新⇒listが2以上の判定⇒AIからのAnswerで追加。AIからのAnswerが1マイクロ秒で終わったりはしないので、この判定で十分正しい
    ...
    public static void RemoveEarliestQandA()
    {
        lock (lockContents)
        {
            // 先頭の一つはシステムメッセージなので、index[0]とindex[1]を削除する。
            if (_contents.Count >= 2)
            {
                _contents.RemoveRange(0, 2);
            }
            lastDeleteTime = DateTime.Now;
